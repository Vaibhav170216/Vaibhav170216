# Hi there, I'm Vaibhav Nagar 👋

<div align="center">
  
### ML Engineer | LLMs | GeoAI |


[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/vaibhav-nagar-a81176191)
[![X](https://img.shields.io/badge/X-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/VNagar24040)

</div>

---

## 🚀 About Me

-  M.Tech Graduate from NIT Warangal with a background in Remote Sensing and GIS with focus on **GeoAI**
-  Interested in Foundation Models and Multimodal AI
-  Learning and applying **MLOps best practices** and automated ML pipelines
-  Ask me about **LLMs, RAG, Transformers or Geospatial deep learning**

---

## 🛠️ Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white)

### ML/DL Frameworks
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![Hugging Face](https://img.shields.io/badge/🤗_Hugging_Face-FFD21E?style=for-the-badge)

### MLOps & Tools
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![MLflow](https://img.shields.io/badge/MLflow-0194E2?style=for-the-badge&logo=mlflow&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Weights & Biases](https://img.shields.io/badge/Weights_&_Biases-FFBE00?style=for-the-badge&logo=weightsandbiases&logoColor=white)

### Cloud & Platforms
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Google Cloud](https://img.shields.io/badge/Google_Cloud-4285F4?style=for-the-badge&logo=google-cloud&logoColor=white)

---

## 📊 Featured Projects

###  [GeoAI Research Assistant](https://github.com/Vaibhav170216/geoai-research-assistant)
End-to-end RAG system for exploring GeoAI, Earth Observation and Remote Sensing research papers. Hybrid retrieval (SQLite FTS + semantic search fused via RRF), Streamlit UI, a full RAGAS evaluation pipeline and monitoring with Grafana.

**Tech:** `RAG` `Hybrid Search` `LangChain` `Streamlit` `RAGAS` `Grafana`

###  [Deep Learning for Earth Observation](https://github.com/Vaibhav170216/Masters-Thesis-GeoAI-1D-CNN)
Compared multiple CNN architectures for Sentinel-2 Land Use/Land Cover classification, achieving a 90% F1-score using a ResNeXt-based 1D CNN and presenting the work at ICGCE 2024.

**Tech:** `Deep Learning` `Geospatial Analysis` `Remote Sensing` `1D CNN`

###  [Transformers & Vision Transformers from Scratch](https://github.com/Vaibhav170216/transformers-and-vit-from-scratch)
Built transformer and Vision Transformer (ViT) architectures from scratch in PyTorch. Deep dive into attention mechanisms, positional encodings and patch embeddings.

**Tech:** `PyTorch` `Transformers` `Vision Transformer (ViT)` `NLP`

###  [LLM Fine-tuning with LoRA](https://github.com/Vaibhav170216/Finetuning-Llama-3-on-Dolly-Dataset-with-LoRA)
Fine-tuning Llama 3 on the Dolly dataset using Low-Rank Adaptation (LoRA) for parameter-efficient training. Exploring efficient fine-tuning techniques for large language models.

**Tech:** `Llama 3` `Weights and Biases` `LoRA`

---

## ⭐ Few Repos I'm Currently Following

Some interesting projects and resources I've been exploring:

- [**ucam-eo/tessera**](https://github.com/ucam-eo/tessera) - TESSERA is a GeoAI foundation model that can process time-series satellite imagery
- [**VMarsocci/pangaea-bench**](https://github.com/VMarsocci/pangaea-bench) - A benchmark for evaluation of Geospatial Foundation Models
- [**DataTalksClub/mlops-zoomcamp**](https://github.com/DataTalksClub/mlops-zoomcamp) - MLOps course from DataTalks.Club
- [**DataTalksClub/llm-zoomcamp**](https://github.com/DataTalksClub/llm-zoomcamp) - LLM course including RAGs and Agents

---

## 🎯 Current Focus

-  Developing **RAG** and **Agentic AI** applications
-  Exploring **GeoAI foundation models**

---

## 🤝 Let's Connect!

I'm actively seeking **Machine Learning/AI Engineer roles**. If you're building impactful AI systems or conducting research in these areas, I'd love to connect and explore how I can contribute.

-  [LinkedIn](https://www.linkedin.com/in/vaibhav-nagar-a81176191)
-  [X/Twitter](https://x.com/VNagar24040)
-  [vaibnag11@gmail.com](mailto:vaibnag11@gmail.com)

---

<div align="center">

</div>
